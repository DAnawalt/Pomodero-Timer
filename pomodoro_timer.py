...
SCAN      = "#1a1a28"
DIM       = "#3a3a55"
BTN_SURFACE = "#252538"   # Elevated button bg — clearly distinct from BG/SURFACE
BTN_MINT    = "#1a9970"   # Dark mint for button labels
BTN_AMBER   = "#a07828"   # Dark amber for button labels
BTN_TEXT    = "#555577"   # Dark neutral for RESET / PAUSE / OFF states
...
# ── macOS system sound paths (/System/Library/Sounds/*.aiff) ──────────────────
MACOS_SOUND_DIR = "/System/Library/Sounds"
SND_COMPLETE    = os.path.join(MACOS_SOUND_DIR, "Glass.aiff")
SND_BELL        = os.path.join(MACOS_SOUND_DIR, "Ping.aiff")
SND_INFO        = os.path.join(MACOS_SOUND_DIR, "Tink.aiff")
...
class SoundEngine:
    ...
    def _play(self, path: str):
        """Play any audio file using macOS afplay (supports WAV, AIFF, MP3, etc.)."""
        if self._muted:
            return
        threading.Thread(
            target=lambda: subprocess.run(
                ["afplay", path], stderr=subprocess.DEVNULL
            ),
            daemon=True,
        ).start()

    def _play_system(self, *candidates: str):
        """Try system sound candidates in order; fall back to generated WAV."""
        if self._muted:
            return
    ...
    def session_complete(self):
        self._play_system(SND_COMPLETE, SND_BELL)

    def break_complete(self):
        self._play_system(SND_INFO, SND_BELL)
...
def _notify(title: str, body: str, subtitle: str = ""):
    """
...
class PomodoroApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Button(
            ctrl, text="↺ RESET", bg=BTN_SURFACE, fg=BTN_TEXT,
            font=("Courier New", 10, "bold"), width=12, pady=8,
        ...
        self._btn_tick = tk.Button(
            ctrl2, text="🔔 TICK: ON", bg=BTN_SURFACE, fg=BTN_MINT,
        ...
        self._btn_mute = tk.Button(
            ctrl2, text="🔊 SOUND: ON", bg=BTN_SURFACE, fg=BTN_MINT,
            font=("Courier New", 9), width=13, pady=5,
        ...
        self._btn_export = tk.Button(
            ctrl3, text="💾 EXPORT CSV", bg=BTN_SURFACE, fg=BTN_AMBER,
    ...
    def _export_csv(self):
        ...
        # macOS: export to ~/Desktop for easy access
        ...
        try:
            ...
            self._lbl_export_status.config(
                text=f"✓ Saved {count} sessions ({work_mins:.0f} min work) → Desktop/{filename}",
                fg=MINT,
            )
        except OSError as e:
    ...
    def _refresh_display(self):
        ...
        self._btn_start.config(
            ...
            bg=self._color if not self._running else BTN_SURFACE,
